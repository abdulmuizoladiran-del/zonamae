...
function BottomNav() { return <nav className="fixed bottom-0 left-0 right-0 z-50 border-t border-gold/20 bg-[#090909]/95 px-2 pb-[max(0.5rem,env(safe-area-inset-bottom))] pt-2 backdrop-blur-xl md:hidden"><div className="mx-auto flex max-w-lg items-center justify-around"><BottomNavItem icon={Home} label="Home" to="/" /><BottomNavItem icon={CrownIcon} label="VIP" to="/vip-membership" /><BottomNavItem icon={Film} label="Videos" to="/vip-videos" active /><BottomNavItem icon={TruckIcon} label="Track Card" to="/track-card" /><BottomNavItem icon={UserRound} label="Profile" to="/login" /></div></nav>; }
function CrownIcon() { return <span className="font-display text-lg leading-none">♛</span>; }
function TruckIcon() { return <span className="text-sm leading-none">▣</span>; }
