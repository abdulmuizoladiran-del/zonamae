...
const benefits = ["Official Bailey VIP Access", "Exclusive News & Updates", "Early Ticket Access", "Priority Support", "Members-Only Events", "Exclusive Merchandise"];
const plans = [
  { name: "Silver VIP", amount: 500, discountedAmount: 200, accent: false, items: ["Official Bailey VIP Access", "Exclusive News & Updates", "Early Ticket Access", "Priority Support"] },
  { name: "Gold VIP", amount: 1500, discountedAmount: 500, accent: true, items: ["All Silver VIP Benefits", "Meet & Greet Access", "Signed Merchandise", "Birthday Message from Bailey", "Exclusive Videos & Photos"] },
  { name: "Platinum VIP", amount: 3000, discountedAmount: 800, accent: false, items: ["All Gold VIP Benefits", "Backstage Access", "Personalized Video Message", "VIP Concierge Support", "Exclusive Invitations"] },
];
const testimonials = [
  { quote: "Being a VIP member has given me experiences I never thought possible. Thank you Bailey and the entire team!", name: "Sarah M.", role: "Gold VIP Member", avatar: "https://images.unsplash.com/photo-1544005313-94ddf0286df2?q=80&w=200&auto=format&fit=crop" },
  ...
  { quote: "My VIP access arrived quickly and the exclusive content is worth every penny. I love this community.", name: "Amara R.", role: "Silver VIP Member", avatar: "https://images.unsplash.com/photo-1531123897727-8f129e1688ce?q=80&w=200&auto=format&fit=crop" },
];

export default function VipMembership() {
  const [couponApplied, setCouponApplied] = useState(false);
  return <Layout><VipHero /><WhyVip /><Benefits /><Plans onCouponChange={setCouponApplied} /><Compare couponApplied={couponApplied} /><Process /><Events /><Testimonials /><Faq /><Registration couponApplied={couponApplied} /><Trust /><FinalCta /></Layout>;
}
...
function VipHero() {
  return <section className="relative overflow-hidden border-b border-border bg-noise py-14 sm:py-20"><div className="pointer-events-none absolute right-0 top-0 h-[500px] w-[500px] rounded-full bg-gold/10 blur-[130px]" /><div className="container relative grid items-center gap-12 lg:grid-cols-[0.95fr_1.05fr]"><div className="text-center lg:text-left"><p className="text-xs font-semibold uppercase tracking-[0.28em] text-gold">Become an Official</p><h1 className="mt-3 font-display text-5xl font-bold uppercase leading-[0.95] text-gradient-gold sm:text-7xl">VIP Member</h1><p className="mt-4 text-sm font-semibold uppercase tracking-[0.18em] text-white">Exclusive access. Unforgettable experiences.</p><p className="mx-auto mt-5 max-w-md text-sm leading-relaxed text-muted-foreground lg:mx-0">Join thousands of loyal fans and unlock a world of exclusive content, VIP benefits, and unforgettable moments with Bailey.</p><div className="mt-7 flex flex-col justify-center gap-3 sm:flex-row lg:justify-start"><a href="#plans" className="flex items-center justify-center gap-2 rounded-full bg-gradient-to-r from-gold-light to-gold px-7 py-3.5 text-xs font-bold uppercase tracking-wider text-black transition-transform hover:scale-105"><Crown className="h-4 w-4" />Explore VIP Access</a><a href="#benefits" className="flex items-center justify-center gap-2 rounded-full border border-gold/60 px-7 py-3.5 text-xs font-bold uppercase tracking-wider text-gold transition-colors hover:bg-gold/10">Learn More</a></div><div className="mt-7 flex items-center justify-center gap-3 lg:justify-start"><div className="flex -space-x-2">{testimonials.map((t) => <img key={t.name} src={t.avatar} alt="" className="h-8 w-8 rounded-full border-2 border-background object-cover" />)}</div><div><p className="text-sm font-bold text-white">250K+</p><p className="text-[10px] uppercase tracking-wider text-muted-foreground">Happy VIP Members</p></div></div></div><div className="relative mx-auto w-full max-w-xl"><img src={goldImage} alt="Bailey" className="mx-auto aspect-[1.4/1] w-full rounded-2xl object-cover object-top opacity-90 sm:aspect-[1.65/1]" /><div className="absolute inset-0 rounded-2xl bg-gradient-to-r from-background via-transparent to-transparent" /><div className="absolute -bottom-10 right-0 w-[80%] animate-float sm:-bottom-12 sm:w-[62%]"><img src="https://cdn.builder.io/api/v1/image/assets%2F387c17f062d04bd5837dfeec0d29ae1e%2F3cf1ee6a5c104470871b22f882725a5a?format=webp&width=800&height=1200" alt="Bailey VIP membership card" className="aspect-[1.5/1] w-full rounded-xl border border-gold/40 object-cover shadow-2xl" /></div><div className="pointer-events-none absolute -bottom-16 left-1/2 h-16 w-[95%] -translate-x-1/2 rounded-[50%] border-2 border-gold/70 shadow-[0_0_45px_12px_hsl(var(--gold)/0.28)]" /></div></div></section>;
}

function WhyVip() {
  const items = [{ icon: Crown, title: "Exclusive Content", copy: "Access behind-the-scenes videos, photos, and updates." }, { icon: Users, title: "Meet & Greet Access", copy: "Get priority access to Meet & Greet events." }, { icon: WalletCards, title: "Official Bailey VIP Access", copy: "Receive your exclusive Bailey VIP access design." }, { icon: Headphones, title: "Priority Support", copy: "Enjoy dedicated support and faster responses." }];
  return <section className="container py-12 sm:py-16"><div className="rounded-2xl border border-border bg-card/50 p-5 sm:p-7"><SectionHeading eyebrow="Why Join VIP?" title="A closer connection to Bailey" /><div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">{items.map((item) => <div key={item.title} className="rounded-xl border border-border/80 bg-background/40 p-5 transition-all hover:-translate-y-1 hover:border-gold/50 hover:shadow-[0_0_30px_-12px_hsl(var(--gold)/0.4)]"><item.icon className="h-7 w-7 text-gold" /><h3 className="mt-4 text-sm font-semibold text-white">{item.title}</h3><p className="mt-2 text-xs leading-relaxed text-muted-foreground">{item.copy}</p></div>)}</div></div></section>;
}

function Benefits() {
  return <section id="benefits" className="container py-10 sm:py-14"><div className="rounded-2xl border border-border bg-card/50 p-5 sm:p-7"><SectionHeading eyebrow="Membership Benefits" title="More than a membership. It's your backstage pass." copy="Every tier is built to make you feel closer to the music, the moments, and the Bailey community." /><div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">{benefits.map((item) => <div key={item} className="flex items-center gap-3 rounded-lg border border-border/70 bg-card/40 p-3 text-sm text-white"><span className="flex h-6 w-6 shrink-0 items-center justify-center rounded-full bg-gold/15 text-gold"><Check className="h-3.5 w-3.5" /></span>{item}</div>)}</div></div></section>;
}

function Plans({ onCouponChange }: { onCouponChange: (applied: boolean) => void }) {
  const [coupon, setCoupon] = useState("");
  const [couponApplied, setCouponApplied] = useState(false);
  const [couponError, setCouponError] = useState(false);
  const applyCoupon = () => {
    const valid = coupon.trim().toLowerCase() === "baiblue27";
    setCouponApplied(valid);
    setCouponError(!valid);
    onCouponChange(valid);
  };
  return <section id="plans" className="container py-12 sm:py-16"><div className="flex flex-wrap items-end justify-between gap-4"><SectionHeading eyebrow="Membership Plans" title="Choose your level of access" /></div><div className="mb-8 rounded-2xl border border-gold/30 bg-gold/5 p-5"><p className="text-sm font-semibold text-white">Have a coupon code?</p><div className="mt-3 flex flex-col gap-2 sm:flex-row"><input value={coupon} onChange={(event) => setCoupon(event.target.value)} aria-label="Coupon code" placeholder="Enter coupon code" className="min-w-0 flex-1 rounded-full border border-border bg-background px-4 py-3 text-sm text-white outline-none focus:border-gold" /><button type="button" onClick={applyCoupon} className="rounded-full bg-gradient-to-r from-gold-light to-gold px-6 py-3 text-xs font-bold uppercase tracking-wider text-black transition-transform hover:scale-[1.02]">Apply</button></div>{couponError && <p className="mt-2 text-xs text-red-300">Invalid coupon code.</p>}{couponApplied && <p className="mt-2 text-xs text-gold">Coupon applied. Discounted pricing unlocked.</p>}</div><div className="grid gap-5 lg:grid-cols-3">{plans.map((plan) => <div key={plan.name} className={cn("relative rounded-2xl border border-border bg-card/50 p-6 transition-all hover:-translate-y-1 hover:border-gold/50", plan.accent && "border-gold shadow-[0_0_45px_-15px_hsl(var(--gold)/0.65)]")}>
    ...
    <div className="flex items-center justify-between"><div><h3 className="font-display text-2xl text-gold">{plan.name}</h3><p className="mt-1 text-[9px] font-bold uppercase tracking-[0.18em] text-muted-foreground">ONE-TIME MEMBERSHIP</p></div><Gem className="h-6 w-6 text-gold/60" /></div><p className="mt-4 font-display text-3xl text-white">{couponApplied && <span className="mr-2 text-lg text-muted-foreground line-through">${plan.amount}</span>}<span>${couponApplied ? plan.discountedAmount : plan.amount}</span></p><ul className="mt-6 space-y-3">{plan.items.map((item) => <li key={item} className="flex gap-2 text-xs text-muted-foreground"><Check className="h-3.5 w-3.5 shrink-0 text-gold" />{item}</li>)}</ul><a href="#registration" className={cn("mt-7 flex items-center justify-center rounded-full border border-gold/60 py-3 text-xs font-bold uppercase tracking-wider text-gold transition-colors hover:bg-gold/10", plan.accent && "border-transparent bg-gradient-to-r from-gold-light to-gold text-black hover:brightness-110")}>Join {plan.name.replace(" VIP", " Gold").replace("Gold Gold", "Gold")}</a></div>)}</div></section>;
}

function Compare({ couponApplied }: { couponApplied: boolean }) {
  const rows = [["VIP Access", "✓", "✓", "✓"], ["Exclusive Content", "✓", "✓", "✓"], ["Early Ticket Access", "✓", "✓", "✓"], ["Meet & Greet", "—", "✓", "✓"], ["Backstage Access", "—", "—", "✓"], ["Signed Merchandise", "—", "✓", "✓"], ["Personalized Message", "—", "—", "✓"], ["ONE-TIME PAYMENT", ...(couponApplied ? ["$200", "$500", "$800"] : ["$500", "$1500", "$3000"])] ];
...
function Process() {
  const steps = [{ icon: UserRound, title: "Apply", copy: "Submit your application" }, { icon: Mail, title: "Review", copy: "We review your details" }, { icon: WalletCards, title: "Payment", copy: "Secure payment" }, { icon: Check, title: "Approval", copy: "You get approved" }, { icon: Package, title: "VIP Access", copy: "We prepare your access" }, { icon: Crown, title: "Welcome", copy: "Enjoy VIP benefits!" }];
...
function handleRegistrationSubmit(event: FormEvent<HTMLFormElement>, couponApplied: boolean) {
  ...
  const selectedPlan = plans.find((item) => plan.startsWith(item.name))!;
  const paymentAmount = couponApplied ? selectedPlan.discountedAmount : selectedPlan.amount;
  ...
  const message = [
    ...
    "",
    "💳 Payment Amount:",
    `$${paymentAmount} — ONE-TIME PAYMENT`,
    "",
    "🔒 Billing:",
    "ONE-TIME PAYMENT ONLY",
    "",
    "🎟️ Coupon Code:",
    couponApplied ? "baiblue27" : "None",
    "",
    "🎂 Date of Birth:",
    dateOfBirth,
    "",
...
function Registration({ couponApplied }: { couponApplied: boolean }) {
  return <section id="registration" className="container py-12 sm:py-16"><div className="grid gap-8 lg:grid-cols-[1fr_1.25fr]"><div><SectionHeading eyebrow="Join VIP Membership" title="Your exclusive access starts here" copy="Complete your details and continue to secure payment. A confirmation will be sent to your email." /><div className="rounded-xl border border-gold/30 bg-gold/5 p-5"><p className="flex items-center gap-2 text-sm font-semibold text-gold"><Lock className="h-4 w-4" /> Secure application</p><p className="mt-2 text-xs leading-relaxed text-muted-foreground">Your information is encrypted and protected. You can cancel anytime.</p></div></div><form onSubmit={(event) => handleRegistrationSubmit(event, couponApplied)} className="grid gap-4 rounded-2xl border border-border bg-card/50 p-5 sm:grid-cols-2 sm:p-7"><label className="text-xs text-muted-foreground">Full Name<input required name="fullName" className="mt-2 w-full rounded-lg border border-border bg-background px-3 py-3 text-sm text-white outline-none focus:border-gold" placeholder="Your full name" /></label><label className="text-xs text-muted-foreground">Email Address<input required name="email" type="email" className="mt-2 w-full rounded-lg border border-border bg-background px-3 py-3 text-sm text-white outline-none focus:border-gold" placeholder="you@example.com" /></label><label className="text-xs text-muted-foreground">Phone Number<input required name="phone" className="mt-2 w-full rounded-lg border border-border bg-background px-3 py-3 text-sm text-white outline-none focus:border-gold" placeholder="+1 000 000 0000" /></label><label className="text-xs text-muted-foreground">Country<select required name="country" className="mt-2 w-full rounded-lg border border-border bg-background px-3 py-3 text-sm text-white outline-none focus:border-gold"><option>United States</option><option>United Kingdom</option><option>France</option><option>United Arab Emirates</option></select></label><label className="text-xs text-muted-foreground">Preferred Plan<select required name="plan" className="mt-2 w-full rounded-lg border border-border bg-background px-3 py-3 text-sm text-white outline-none focus:border-gold"><option>Gold VIP — ${couponApplied ? "500" : "1500"} — ONE-TIME PAYMENT</option><option>Silver VIP — ${couponApplied ? "200" : "500"} — ONE-TIME PAYMENT</option><option>Platinum VIP — ${couponApplied ? "800" : "3000"} — ONE-TIME PAYMENT</option></select></label><label className="text-xs text-muted-foreground">Date of Birth<input required name="dateOfBirth" type="date" className="mt-2 w-full rounded-lg border border-border bg-background px-3 py-3 text-sm text-white outline-none focus:border-gold" /></label><label className="text-xs text-muted-foreground sm:col-span-2">Instagram / Social Media (Optional)<input name="instagram" className="mt-2 w-full rounded-lg border border-border bg-background px-3 py-3 text-sm text-white outline-none focus:border-gold" placeholder="@yourhandle" /></label><label className="flex items-start gap-2 text-[11px] text-muted-foreground sm:col-span-2"><input required type="checkbox" className="mt-0.5 accent-yellow-500" />I agree to the Terms & Conditions and Privacy Policy.</label><button type="submit" className="flex items-center justify-center gap-2 rounded-full bg-gradient-to-r from-gold-light to-gold py-3.5 text-xs font-bold uppercase tracking-wider text-black transition-transform hover:scale-[1.02] sm:col-span-2">Continue to Payment <ArrowRight className="h-4 w-4" /></button></form></div></section>;
}
...
function FinalCta() {
  return <section className="container pb-20 pt-10 sm:pb-28"><div className="relative overflow-hidden rounded-2xl border border-gold/40 bg-gradient-to-r from-gold/15 via-card to-gold/10 p-8 text-center sm:p-14"><Sparkles className="absolute left-8 top-8 h-10 w-10 text-gold/20" /><PartyPopper className="absolute bottom-8 right-8 h-10 w-10 text-gold/20" /><h2 className="font-display text-3xl font-bold text-white sm:text-4xl">Your VIP story starts now</h2><p className="mx-auto mt-3 max-w-xl text-sm text-muted-foreground">Join the official Bailey community and unlock experiences made exclusively for you.</p><a href="#plans" className="mt-7 inline-flex items-center gap-2 rounded-full bg-gradient-to-r from-gold-light to-gold px-8 py-4 text-xs font-bold uppercase tracking-wider text-black transition-transform hover:scale-105"><Crown className="h-4 w-4" />Join the VIP Community</a></div></section>;
}
